                ...
                <div class="row">
                    ...
                    <div class="col-sm-9">
                        <div class="row">
                            <div class="col-sm-4">
                                <div class="form-group form-group-sm">
                                    <select id="category" class="form-control"style="min-width: 150px;">
                                        ...
                                        <?php foreach ($page_categories as $pc_key => $pc_value): ?>
                                        <option value="<?php echo $pc_key; ?>" <?php echo $pc_key==CT_PAGECATEGORY_PRODUCT?'selected':''; ?>><?php echo $pc_value; ?></option>
                                        <?php endforeach;?>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-1">
                                <a class="btn btn-sm btn-primary" data-toggle="tooltip" title="Create" href="<?php echo get_cms_url('page/edit'); ?>"><i class="fa fa-plus-square"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    var MenuManager = {
        ...
        load: function (){
            ...
            //load from service
            $.getJSON(_this._getUrl('service/page'),{limit:_this.dataLimit,page:_this.page,category:_this.category,search:_this.searchText}, function(data){
                ...
                if (data.items.length > 0){
                    for (var i in data.items){
                        var s = '<tr id="'+data.items[i].id+'" data-id="'+data.items[i].id+'">';
                            ...
                            s+= '<td>'+data.items[i].date_time+'</td>';
                            s+= '<td>'+data.items[i].link+'</td>';
