    ...
    <div class="col-sm-12">
        ...
        <div class="box">
            <div class="box-header">
                <div class="row">
                    <div class="col-sm-3">
                        <h3 class="box-title">List of Dropdown Links</h3>
                    </div>
                    <div class="col-sm-9">
                        <div class="row">
                            <div class="col-sm-4">
                                <div class="form-group form-group-sm">
                                    <select id="category" class="form-control"style="min-width: 150px;">
                                        ...
                                        <?php foreach ($categories as $dc): ?>
                                        <option value="<?php echo $dc->id; ?>"><?php echo $dc->label_id; ?> / <?php echo $dc->label_en; ?></option>
                                        <?php endforeach;?>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-1">
                                <a class="btn btn-sm btn-primary" data-toggle="tooltip" title="Create" href="<?php echo get_cms_url('dropbox/edit'); ?>"><i class="fa fa-plus-square"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    var MenuManager = {
        ...
        load: function (){
            ...
            //load from service
            $.getJSON(_this._getUrl('service/dropbox'),{limit:_this.dataLimit,page:_this.page,category:_this.category,search:_this.searchText}, function(data){
                ...
                if (data.items.length > 0){
                    for (var i in data.items){
                        var s = '<tr id="'+data.items[i].id+'" data-id="'+data.items[i].id+'">';
                            ...
                            s+= '<td>'+data.items[i].label_id+' / '+data.items[i].label_en+'</td>';
                            s+= '<td>'+data.items[i].category_name+'</td>';
                            s+= '<td>'+data.items[i].href+'</td>';
                            s+= '<td>'+data.items[i].created_dt+'</td>';
                            
                            s+= '<td class="text-center">';
                                s+= '<a class="btn btn-xs btn-warning" data-toggle="tooltip" title="Edit" href="'+(_this._getCMSUrl('dropbox/edit/'+data.items[i].id+'/'+_this.page))+'"><i class="fa fa-pencil-square"></i></a>';
                    ...
                    _this.reachLimit = true;
                    $('#data-list').append('<tr><td colspan="6">No data</td></tr>');
                    console.log('Reach limit');
                }
